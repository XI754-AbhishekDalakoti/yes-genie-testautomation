...
Feature: Validate nba APIs
  ...
  Scenario: User validates the response code of create lead product name api and product name as response
    Given a genie user has a create lead product name api to test is "http://cust360.yesgenie.com:30978/api/lead/productnames"
  ...
  Scenario: User validates the response code of create lead branchcodes api and branchCode, branchDisplayName as response
    Given a genie user has a create lead branchcodes api to test is "http://cust360.yesgenie.com:30978/api/lead/branchcodes"
  ...
  Scenario: User validates the response code of nba create lead without recommendation put api
    Given a genie user has a nba create lead without recommendation api to test is "http://cust360.yesgenie.com:30978/api/lead"
